...
void main() {
  ...
  group("getFrequenciesStream -", () {
    test("should call the event channel and return a stream of data", () async {
      ...
      //act
      final stream = await platformDataSource.getFrequenciesStream();
  ...
  group("getFrequencyBandsStream -", () {
    test("should return expected result when success ", () async {
      ...
      //act
      final result = await platformDataSource.getFrequencyBandsStream();
      final firstElement = await result.first;

      //assert
      expect(firstElement, isA<VisualizerFrequencyBandsModel>());
