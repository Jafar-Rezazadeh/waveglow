...
void main() {
  ...
  late TracksListGetFavoriteSongsUC useCase;

  setUp(() {
    mockTracksListRepository = _MockTracksListRepository();
    useCase = TracksListGetFavoriteSongsUC(repository: mockTracksListRepository);
  });

  test("should call expected method of repository when invoked", () async {
    //arrange
    when(() => mockTracksListRepository.getFavoriteSongs()).thenAnswer((_) async => right([]));

    //act
    await useCase.call(NoParams());

    //assert
    verify(() => mockTracksListRepository.getFavoriteSongs()).called(1);
  });
}
